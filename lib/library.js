...
function generateNewInstance(state, metabase, indent, classname, cast, varname, methodnode) {
	...
	if (typeobj.isWindowsDelegate() && !method) {
		...
		code.push('{');
		code.push(indent+'JSObjectRef callback = JSValueToObject(ctx,arguments[1],NULL);');
		code.push(indent+'instance = ref new '+typeobj.toClassName()+'([ctx,callback]('+callstr.join(',')+') {');
		code.push(indent+'});');
		code.push('}');
