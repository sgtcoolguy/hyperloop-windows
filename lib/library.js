...
function generateMethodArguments(options,metabase,state,method,typelib,instance,code,cleanup,indent) {
	...
	method.args.forEach(function(m,i){
		...
		if (typeobj.isWindowsDelegate()) {
			...
			code.push('\t{');
			code.push('\t\tJSValueProtect(ctx, arguments['+starti+']); // we should keep this until remove_'+method.name.replace(/^add_/,'')+' is called');
			code.push('\t\tJSValueRef js'+value+' = '+constructor+'(ctx,function,thisObject,argumentCount,arguments,exception);');
			code.push('\t\t'+value+' = '+delegate_body+';');
			code.push('\t}');
			code.push('\telse');
			code.push('\t{');
...
function generateNewInstance(state, metabase, indent, classname, cast, varname, methodnode) {
	...
	if (typeobj.isWindowsDelegate() && !method) {
		...
		/*
		// FIXME: can't resolve dependencies for callback
		// - Windows metabase uses 'Invoke' method to describe argument classes
		...
		*/
		...
		code.push('\tinstance = ref new '+typeobj.toClassName()+'([gctx,ggctx,callback]('+callstr.join(',')+') {');
		/*
		...
		*/
		code.push('\t\tJSValueRef result = JSObjectCallAsFunction(gctx, callback, callback, 0, NULL, NULL);');
		...
		code.push('instance = ref new '+typeobj.toClassName()+'('+selector+');');
		...
		if (typeinfo.methods && typeinfo.methods['.ctor']) {
			...
			code.push('Platform::Object^ instance = nullptr;');
		} else {
			code.push(cast+' instance = nullptr;');
		}
	}
