...
function parsePackToken(node){
}
function parseSizeToken(node){
}
...
function parseMethodToken(node) {
	...
	if (obj.name in node.parent.methods) {
		node.parent.methods[obj.name].push(obj);
	}
	else {
		node.parent.methods[obj.name] = [obj];
	}
	node.children = null;
	return true;
}
