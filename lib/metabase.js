...
function loadMetabase(options, arch, callback) {
	...
	// include system header paths
	addToIncludePath(options, hparser.findHeader(options, 'winerror.h'));
	addToIncludePath(options, hparser.findHeader(options, 'winstring.h'));
	addToIncludePath(options, hparser.findHeader(options, 'ppltasks.h'));
...
function addToIncludePath(options,headerPath) {
	var dirname = path.dirname(headerPath);
	if (options.I.indexOf(dirname) < 0) {
		options.I.push(dirname);
	}
}
...
function loadHeaderFile(options,metabase,name,callback) {
	...
	addToIncludePath(options, headerPath);
