...
function getLibraryOutputPath(options) {
	return path.join(getMSBuildBase(options),'hyperloop',getMSBuildConfiguration(options));
}
...
function generateApp(options, arch_results, settings, callback) {
	var templatedir = path.join(__dirname, '..', 'templates','vs12update2', 'main'),
		...
		applib_windows = applib + '.Windows',
		applib_windowsphone = applib + '.WindowsPhone',
	...
	var filterValues = {
		...
		'\\$HYPERLOOP_LIB\\$': path.resolve(getLibraryOutputPath(options)),
