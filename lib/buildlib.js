...
function generateLibrary(options, callback) {
	...
	copyJavaScripCoreHeaderFiles(jsCoreDirFrom, jsCoreDirTo);
...
function copyJavaScripCoreHeaderFiles(jsCoreDirFrom, jsCoreDirTo) {
	...
	fs.readdirSync(jsCoreDirFrom).forEach(function(f) {
		if (headerFilesRE.test(f)) {
			var from = path.join(jsCoreDirFrom, f),
				to = path.join(jsCoreDirTo, path.basename(f));
			log.debug('Copying',f,'to',jsCoreDirTo);
			fs.writeFileSync(to, fs.readFileSync(from));
		}
	});
}
...
function generateApp(options, arch_results, settings, callback) {
	...
	// copy hyperloop header files
	fs.writeFileSync(path.join(copybase,'app.h'), fs.readFileSync(path.join(getMSBuildBase(options), libapp, options.name, 'app.h')));
	fs.writeFileSync(path.join(copybase, 'hyperloop.h'), fs.readFileSync(path.join(getMSBuildBase(options), 'hyperloop', 'hyperloop', 'hyperloop.h')));
	...
	var filterValues = {
		...
		'\\$APP_LIB\\$': path.resolve(path.join(getMSBuildBase(options), libapp, configuration, options.name))
	};
