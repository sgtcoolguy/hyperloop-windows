...
exports.getPlatformTarget = getPlatformTarget;
...
function generateLibrary(options, callback) {
	...
	msbuildproject(options, solutionFile, 'hyperloop', callback);
}

function msbuildproject(options, solutionFile, name, callback) {
	var targetname = name+'.'+options.target,
		basedir = path.dirname(solutionFile),
		projfile = path.join(basedir, name, targetname, targetname+'.vcxproj'),
		configuration = getMSBuildConfiguration(options),
		target = getPlatformTarget(options),
		vsargs = ' /t:Clean;Build /p:OutDir='+path.resolve(path.join(basedir,target,configuration))+';Configuration='+configuration+';Platform='+target;
	programs.msbuild(projfile+vsargs, callback, options);
}

function getPlatformTarget(options) {
...
function getLibraryOutputPath(options, name) {
	var base = path.join(getMSBuildBase(options),name),
		dest = path.join(getPlatformTarget(options),getMSBuildConfiguration(options));
...
function generateApp(options, arch_results, settings, callback) {
	...
	// APPGUID will be updated later on at package command
	var filterValues = {
		'\\$APPNAME\\$':options.name,
		...
		'\\$PHONEPUBLISHERID\\$': options.phonepublisherid,
		'\\$BUILD_PLATFORM\\$': getPlatformTarget(options).toLowerCase()
	};
...
function generateAppLib(options, arch_results, settings, callback) {
	...
	msbuildproject(options, solutionFile, options.name, callback);
}
