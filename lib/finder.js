...
var programFiles = process.env['programfiles'],
	...
	envPaths = process.env.path && process.env.path.split(';'),
...
function find(name, sdk) {
	...
	var found = findFileInPaths(paths, name, cacheKey);
	if (!found) {
		// try the other
		found = findFileInPaths(!sdk ? searchPathsBySDK[sdk] : searchPaths, name, cacheKey);
	}
	return found;
}
